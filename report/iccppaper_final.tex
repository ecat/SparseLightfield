...
\end{center}
   \caption{Comparison of sparsity in angular direction of two images. The left one ( fence image) is an example whose angular images are not very sparse. The right image (cherry blossom image ) is an example whose angular images are sparse. For each image, from bottom to top, the patches in each line represent the original angular image and guesses made by different reconstruction method.}
\end{figure}
...
\end{center}
   \caption{Reconstruction SNR using TV prior and basis pursuit in DCT, FFT and Haar Wavelets. }
\end{figure}
